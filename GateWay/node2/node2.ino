...
const char* topic = "Garden/data_2"; //publish topic
...
void reconnect() {
  while (!client.connected()) {
    if (client.connect(clientID, mqttUserName, mqttPwd)) {
      ...
      client.subscribe("Device/lights_2");
      Serial.println("Topic lights 2 Subscribed");
      client.subscribe("Device/pump_2");
      Serial.println("Topic pump 2 Subscribed");
      client.subscribe("Device/mist_2");
      Serial.println("Topic quat 2 Subscribed");
      client.subscribe("Device/time_2");
      Serial.println("Topic time 2 Subscribed");
      client.subscribe("Garden/mode_2");
...
void callback(char*topic, byte* payload, unsigned int length) {
  ...
  Serial.println(data);
  //---light---
  if (strcmp(topic, "Device/lights_2") == 0) {
    if (data == "ON_L") {
      Serial.print("LIGHT: ");
  ...
  //--- pump----
  else if (strcmp(topic, "Device/pump_2") == 0) {
    if (data == "ON_P") {
      Serial.print("PUMP: ");
  ...
  //---- phun suong ----
  else if (strcmp(topic, "Device/mist_2") == 0) {
    if (data == "ON_S") {
  ...
  else if (strcmp(topic, "Device/time_2") == 0) {
  ...
  //----------mode------------------------
  else if (strcmp(topic, "Garden/mode_2") == 0) {
    if (data == "AUTO") {
      status_mode = AUTO;
    } else if (data == "MAN") {
      status_mode = MAN;
  ...
  if (status_mode == AUTO) {
    ...
    if (tb_soil <= 40) {
      if (digitalRead(PUMP) == LOW) {
        digitalWrite(PUMP, HIGH) ;
        client.publish("Device/pump_2", "ON_P");
      }
    }
    if (tb_soil >= 60) {
      if (digitalRead(PUMP) == HIGH) {
        digitalWrite(PUMP, LOW);
        client.publish("Device/pump_2", "OFF_P");
      }
    ...
    if (tb_humi >= 80 ) {
      if (digitalRead(SUONG) == LOW) {
        digitalWrite(SUONG, HIGH) ;
        client.publish("Device/mist_2", "ON_S");
      }
    ...
    if (tb_humi >= 50 && tb_humi <= 80) {
      if (digitalRead(SUONG) == HIGH) {
        digitalWrite(SUONG, LOW);
        client.publish("Device/mist_2", "OFF_S");
      }
...
void control_mode() {
  if (status_mode == MAN ) {
    if ((unsigned long) (millis() - time_mode1) > 30000) {
      status_mode = AUTO;
      client.publish("Garden/mode_2", "AUTO");
      time_mode1 = millis();
    }
  }
}
